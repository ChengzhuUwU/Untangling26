...
namespace lcs::Initializer
{
	...
	{
		if (vid1 == vid2)
			LUISA_ERROR("Constraint topology contains a self adjacency at vertex {}.", vid1);
		...
		lcs::SimulationData<std::vector>*							   sim_data)
	{
			...
			[&](const uint eid)
			{
				...
				bool  is_dynamic = cull_unused_constraints ? !mesh_data->sa_is_fixed[edge[0]] || !mesh_data->sa_is_fixed[edge[1]]
						|| !mesh_data->sa_is_fixed[edge[2]] || !mesh_data->sa_is_fixed[edge[3]]
														  : true;
				return (use_bending && is_dynamic && bending_edge_has_valid_geometry(edge, mesh_data->sa_rest_x)) ? 1 : 0;
			},
			mesh_data->num_dihedral_edges);
		const uint invalid_dihedral_count = static_cast<uint>(std::count_if(
			mesh_data->sa_dihedral_edges.begin(), mesh_data->sa_dihedral_edges.end(),
			[&](const uint4 edge)
			{ return !bending_edge_has_valid_geometry(edge, mesh_data->sa_rest_x); }));
		if (invalid_dihedral_count > 0u)
			LUISA_WARNING("Skipped {} invalid dihedral edge(s) with repeated vertices or degenerate geometry.", invalid_dihedral_count);
		...
		// The DOF layout assumes soft vertices occupy the first num_verts_soft
		// mesh slots: init_world_data sorts materials (Cloth < Tetrahedral <
		// Rigid < Rod), so this identity holds unless a material sorting after
		// Rigid shares the scene with rigid bodies. Fail loudly instead of
		// silently scattering inertia gradients to the wrong DOFs.
		for (uint vid = 0; vid < num_verts_soft; ++vid)
		{
			if (soft_vert_indices[vid] != vid)
			{
				LUISA_ERROR(
					"Soft DOF {} maps to mesh vertex {}, but the DOF layout requires the identity mapping "
					"(soft mesh vertices must be the first {} global vertices). Materials sorting after Rigid "
					"(e.g. Rod) cannot be mixed with Rigid bodies in one scene.",
					vid,
					soft_vert_indices[vid],
					num_verts_soft);
			}
		}
		...
		// Init state buffers
		{
			...
			// Init position/velocity
			{
				...
				for (uint body_idx = 0; body_idx < num_affine_bodies; body_idx++)
				{
					...
					sim_data->sa_q_property[dof_idx + 0].set_dof_offset(0);
					sim_data->sa_q_property[dof_idx + 1].set_dof_offset(1);
					sim_data->sa_q_property[dof_idx + 2].set_dof_offset(2);
					sim_data->sa_q_property[dof_idx + 3].set_dof_offset(3);
					if (has_fixed_vert)
					{
		...
		// Init energy
		{
			...
			CpuParallel::parallel_for(0,
				...
				[&](const uint eid)
				{
					...
					stretch_spring_data.sa_stretch_spring_rest_state_length[eid] = std::max(lcs::length_vec(x1 - x2), 1e-6f);
			...
			CpuParallel::parallel_for(0,
				...
				[&](const uint vid)
				{
					...
					soft_inertia_data.sa_stiffness_dirichlet[vid] = is_fixed ? mesh_data->sa_fixed_stiffness[orig_vid] : 1.0f;
				});
			...
			for (uint body_idx = 0; body_idx < num_affine_bodies; body_idx++)
			{
				...
				abd_inertia_data.sa_stiffness_dirichlet[body_idx] = has_fixed_vert ? mesh_data->sa_fixed_stiffness[prefix_vid] : 1.0f;
		...
		lcs::SimulationData<luisa::compute::Buffer>* device_data)
	{
		...
		// PCG block reductions write one entry per 256-thread block for the
		// p^T q pass, but two entries (r^T r and r^T z) per block for the
		// preconditioner pass. The scratch therefore must hold
		// 2 * ceil(num_dof / 256) floats, which num_dof alone does NOT cover
		// for tiny systems (num_dof == 1 would overflow a num_dof-sized buffer).
		const uint pcg_reduction_capacity = 2u * ((num_verts + 255u) / 256u);
		...
		resize_buffer(host_data->sa_block_result, pcg_reduction_capacity);
		...
		resize_buffer(device, device_data->sa_block_result, pcg_reduction_capacity);
