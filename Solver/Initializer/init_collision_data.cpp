...
namespace lcs::Initializer
{
		...
		lcs::CollisionData<std::vector>*							   collision_data)
	{
		...
		std::vector<float> vert_min_dist(mesh_data->num_verts, 1e10f);
		CpuParallel::single_thread_for(0,
			...
			[&](const uint eid)
			{
				...
				if (!mesh_data->sa_is_fixed[edge[0]] && !mesh_data->sa_is_fixed[edge[1]])
				{
					...
					vert_min_dist[edge[0]] = min_scalar(vert_min_dist[edge[0]], rest_length);
					vert_min_dist[edge[1]] = min_scalar(vert_min_dist[edge[1]], rest_length);
		...
		CpuParallel::single_thread_for(0,
			...
			[&](const uint fid)
			{
				...
				if (!mesh_data->sa_is_fixed[face[0]] && !mesh_data->sa_is_fixed[face[1]] && !mesh_data->sa_is_fixed[face[2]])
				{
					...
					for (int i = 0; i < 3; i++)
					{
						vert_min_dist[face[i]] = min_scalar(vert_min_dist[face[i]], rest_dist);
					}
		...
		constexpr bool use_mesh_unified_offset_dhat = false;
		if constexpr (use_mesh_unified_offset_dhat)
		{
			...
			CpuParallel::parallel_for(0,
				...
				[&](const uint vid)
				{
					...
					sim_data->sa_contact_active_verts_friction_coeff[vid] = world_data[mesh_idx].get_friction_mu();
		...
		{
			auto get_safe_dist = [](const uint mesh_idx, const float init_gap)
			{
				const float safe_dist = 0.9f * init_gap;
				return safe_dist;
			};
			auto get_scaled_offset_dhat = [&](const uint mesh_idx, const float init_gap)
			{
				const float safe_dist = get_safe_dist(mesh_idx, init_gap);
				float		desire_offset = 0.5f * world_data[mesh_idx].get_contact_offset();
				float		desire_d_hat = world_data[mesh_idx].get_d_hat();
				if (desire_offset < safe_dist)
				{
					if (desire_d_hat < safe_dist - desire_offset)
					{
						return std::make_pair(desire_offset, desire_d_hat);
					}
					else
					{
						return std::make_pair(desire_offset, safe_dist - desire_offset);
					}
				}
				else
				{
					desire_offset = 0.5f * safe_dist;
					if (desire_d_hat < safe_dist - desire_offset)
					{
						return std::make_pair(desire_offset, desire_d_hat);
					}
			...
			CpuParallel::parallel_for(0, mesh_data->num_verts,
				[&](const uint vid)
				{
					const uint mesh_idx = mesh_data->sa_vert_mesh_id[vid];
					auto [scaled_offset, scaled_d_hat] = get_scaled_offset_dhat(mesh_idx, vert_min_dist[vid]);
					sim_data->sa_contact_active_verts_d_hat[vid] = scaled_d_hat;
					sim_data->sa_contact_active_verts_offset[vid] = scaled_offset;
					sim_data->sa_contact_active_verts_friction_coeff[vid] = world_data[mesh_idx].get_friction_mu();
				});
		}
