	...
	namespace BendingEnergyUtils
	{
		...
		namespace detail
		{
			...
			static inline HostVector12 face_dihedral_angle_grad(const float3& v2, const float3& v0, const float3& v1, const float3& v3)
			{
				...
				const float	 n1_sqnm = std::max(luisa::dot(n1, n1), 1e-12f);
				const float	 n2_sqnm = std::max(luisa::dot(n2, n2), 1e-12f);
				const float	 e0_norm = std::max(luisa::length(e0), 1e-6f);
				...
				const Float3&													v3)
			{
				...
				const Float	 n1_sqnm = luisa::compute::max(luisa::compute::dot(n1, n1), 1e-12f);
				const Float	 n2_sqnm = luisa::compute::max(luisa::compute::dot(n2, n2), 1e-12f);
				const Float	 e0_norm = luisa::compute::max(luisa::compute::length(e0), 1e-6f);
			...
			static inline float face_dihedral_angle(const float3& v0, const float3& v1, const float3& v2, const float3& v3)
			{
				...
				const float	 denom = std::max(luisa::sqrt(luisa::dot(n1, n1) * luisa::dot(n2, n2)), 1e-12f);
				float		 dot = luisa::dot(n1, n2) / denom;
			...
			static inline Float face_dihedral_angle(const Float3& v0, const Float3& v1, const Float3& v2, const Float3& v3)
			{
				...
				const Float	 denom = luisa::compute::max(luisa::compute::sqrt(luisa::compute::dot(n1, n1) * luisa::compute::dot(n2, n2)), 1e-12f);
				Float		 dot = luisa::compute::dot(n1, n2) / denom;
