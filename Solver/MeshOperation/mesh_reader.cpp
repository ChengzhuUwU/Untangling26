...
namespace SimMesh
{
		...
		bool												 extract_bending_edge)
	{
		...
		output_edges.clear();
		output_bending_edges.clear();
		output_edges.reserve(tmp_faces.size());
		if (extract_bending_edge)
			output_bending_edges.reserve(tmp_faces.size() / 2);

		// Group identical undirected edges by (v0, v1).
		// - Always emit exactly one topological edge per group.
		// - Emit dihedral edges for all face pairs around this edge:
		//   if k adjacent faces share this edge, emit C(k,2) dihedral entries.
		//   (k=1 boundary -> 0; k=2 manifold -> 1; k>2 non-manifold -> multiple)
		for (uint begin = 0; begin < tmp_faces.size();)
		{
			const Int3 curr_value = tmp_faces[begin];
			const uint v0 = curr_value[0];
			const uint v1 = curr_value[1];

			uint end = begin + 1;
			while (end < tmp_faces.size())
			{
				const Int3 next_value = tmp_faces[end];
				if (next_value[0] != v0 || next_value[1] != v1)
					break;
				end++;
			}

			output_edges.push_back(Int2{ v0, v1 });

			const uint adj_face_count = end - begin;
			if (extract_bending_edge && adj_face_count >= 2)
			{
				const Int2 dehedral_edge = Int2{ v0, v1 };
				for (uint ii = begin; ii < end; ii++)
				{
					const Int3 face_value_0 = tmp_faces[ii];
					const Int3 face_0 = input_faces[face_value_0[2]];
					const uint rest_vid_0 =
						(face_0[0] + face_0[1] + face_0[2]) - (dehedral_edge[0] + dehedral_edge[1]);
					for (uint jj = ii + 1; jj < end; jj++)
					{
						const Int3 face_value_1 = tmp_faces[jj];
						const Int3 face_1 = input_faces[face_value_1[2]];
						const uint rest_vid_1 =
							(face_1[0] + face_1[1] + face_1[2]) - (dehedral_edge[0] + dehedral_edge[1]);
						output_bending_edges.push_back(Int4{ dehedral_edge[0], dehedral_edge[1], rest_vid_0, rest_vid_1 });
					}
				}
			}
	...
	bool read_mesh_file(std::string_view obj_path, TriangleMeshData& mesh_data)
	{
		...
		uint num_verts = static_cast<uint>(mesh_attrib.vertices.size() / 3);
		uint num_faces = 0;
		...
		// std::vector<bool> vert_accessed(num_verts, false);
		// for (const auto& sub_obj : mesh_shape)
		// {
		// 	const auto& face_list = sub_obj.mesh.indices;
		...
		// 		vert_accessed[v0.vertex_index] = true;
		// 		vert_accessed[v1.vertex_index] = true;
		// 		vert_accessed[v2.vertex_index] = true;
		// 	}
		// }
