...
namespace lcs
{
    ...
    constexpr uint type_ee() { return 1 << 3; }
		...
		struct EfPair
		{
			std::array<uint, 5>	 indices;
			std::array<float, 3> baries;
			std::array<float, 3> gradient;
			float				 stiff;

			uint2 get_edge() const
			{
				return luisa::make_uint2(mask_get_index & indices[0], mask_get_index & indices[1]);
			}
			uint3 get_face() const
			{
				return luisa::make_uint3(
					mask_get_index & indices[2], mask_get_index & indices[3], mask_get_index & indices[4]);
			}
			void set_edge(const uint2& edge)
			{
				indices[0] = edge.x;
				indices[1] = edge.y;
			}
			void set_face(const uint3& face)
			{
				indices[2] = face.x;
				indices[3] = face.y;
				indices[4] = face.z;
			}
			std::array<uint, 5> get_indices() const
			{
				return { indices[0] & mask_get_index,
					indices[1] & mask_get_index,
					indices[2] & mask_get_index,
					indices[3] & mask_get_index,
					indices[4] & mask_get_index };
			}
			std::array<float, 5> get_weights() const
			{
				return { baries[0], 1.0f - baries[0], -baries[1], -baries[2], baries[1] + baries[2] - 1.0f };
			}

			float2 get_edge_bary() const { return luisa::make_float2(baries[0], 1.0f - baries[0]); }
			float3 get_face_bary() const
			{
				return luisa::make_float3(baries[1], baries[2], 1.0f - baries[1] - baries[2]);
			}
			float3 get_gradient() const { return luisa::make_float3(gradient[0], gradient[1], gradient[2]); }
			float  get_stiffness() const { return stiff; }
			bool   get_is_loop_vertex() const { return indices[0] == indices[1]; }
			void   set_stiffness(const float s) { stiff = s; }
			void   set_gradient(const float3& g)
			{
				gradient[0] = g[0];
				gradient[1] = g[1];
				gradient[2] = g[2];
			...
			void make_ef_pair(const uint2& edge, const uint3& face, const float2& edge_bary, const float3& face_bary)
			{
				indices = { edge.x, edge.y, face.x, face.y, face.z };
				baries = { edge_bary.x, face_bary.x, face_bary.y };
...
LUISA_STRUCT(lcs::CollisionPair::CollisionPairTemplate, indices, vec1, vec2, vec3)
{
    ...
    void set_k1(const luisa::compute::Float db_dd)   { vec2[0] = db_dd; }
    void set_k2(const luisa::compute::Float d2b_dd2) { vec2[1] = d2b_dd2; }
...
// clang-format off
LUISA_STRUCT(lcs::CollisionPair::EfPair, indices, baries, gradient, stiff)
{
    luisa::compute::Var<luisa::uint2> get_edge() const { return luisa::compute::make_uint2(indices[0] & lcs::mask_get_index, indices[1] & lcs::mask_get_index); }
    luisa::compute::Var<luisa::uint3> get_face() const { return luisa::compute::make_uint3(indices[2] & lcs::mask_get_index, indices[3] & lcs::mask_get_index, indices[4] & lcs::mask_get_index); }
    luisa::compute::Var<luisa::float2> get_edge_bary() const { return luisa::compute::make_float2(baries[0], 1.0f - baries[0]); }
    luisa::compute::Var<luisa::float3> get_face_bary() const
    {
        return luisa::compute::make_float3(baries[1], baries[2], 1.0f - baries[1] - baries[2]);
    }
    luisa::compute::Var<std::array<uint, 5>> get_indices() const { return {
        indices[0] & lcs::mask_get_index,
        indices[1] & lcs::mask_get_index,
        indices[2] & lcs::mask_get_index,
        indices[3] & lcs::mask_get_index,
        indices[4] & lcs::mask_get_index
    }; }
    luisa::compute::ArrayVar<float, 5> get_weights() const
    {
        return luisa::compute::ArrayVar<float, 5>{
            baries[0],
            1.0f - baries[0],
            -baries[1],
            -baries[2],
            baries[1] + baries[2] - 1.0f};
    }
    luisa::compute::Var<luisa::float3>  get_gradient() const { return luisa::compute::make_float3(gradient[0], gradient[1], gradient[2]); }
    luisa::compute::Var<float>         get_stiffness() const { return stiff; }
	luisa::compute::Var<bool>     get_is_loop_vertex() const { return indices[0] == indices[1]; }
    void  set_stiffness(const luisa::compute::Var<float>& s) { stiff = s; }
    void  set_gradient(const luisa::compute::Var<luisa::float3>& g)
    {
        gradient[0] = g[0];
        gradient[1] = g[1];
        gradient[2] = g[2];
    ...
    void make_ef_pair(  const luisa::compute::Var<luisa::uint2>&  edge,
                        const luisa::compute::Var<luisa::uint3>&  face,
                        const luisa::compute::Var<luisa::float2>& edge_bary,
                        const luisa::compute::Var<luisa::float3>& face_bary)
    {
        indices = {edge.x, edge.y, face.x, face.y, face.z};
        baries = {edge_bary.x, face_bary.x, face_bary.y};
        gradient = {0.0f, 0.0f, 0.0f};
...
namespace lcs
{
	...
	struct CollisionData : SimulationType
	{
		...
		BufferType<uint2>								 narrow_phase_list_indices; // 1
		...
		// Intersection data
		BufferType<uint>				  vert_is_invalid;
		BufferType<uint>				  broad_phase_list_ef;
		BufferType<CollisionPair::EfPair> narrow_phase_list_ef;
		BufferType<uint2>				  narrow_phase_list_ef_indices;
		...
		const uint get_ef_count_offset() { return 4; }
			...
			const uint											   num_dofs)
		{
			...
			lcs::Initializer::resize_buffer(device, this->broad_phase_collision_count, 8);
			lcs::Initializer::resize_buffer(device, this->narrow_phase_collision_count, 12);
			...
			lcs::Initializer::resize_buffer(device, this->toi_per_vert, num_dofs);
			lcs::Initializer::resize_buffer(device, this->vert_is_invalid, num_verts);
		...
		inline size_t get_momery_bytes() const
		{
			const size_t collision_pair_bytes =
				...
				+ sizeof(float) * this->toi_per_vert.size()
				+ sizeof(float) * this->contact_energy.size()
				+ sizeof(CollisionPair::CollisionPairTemplate) * this->narrow_phase_list.size()
				+ sizeof(uint2) * this->narrow_phase_list_indices.size()
				+ sizeof(CollisionPair::EfPair) * this->narrow_phase_list_ef.size()
				+ sizeof(uint2) * this->narrow_phase_list_ef_indices.size()
			...
			const bool												   allocate_triplet)
		{
			...
			if (allocate_contact_list)
			{
				...
				lcs::Initializer::resize_buffer(device, this->broad_phase_list_ef, per_element_count_BP * num_edges);
				...
				lcs::Initializer::resize_buffer(device, this->narrow_phase_list_indices, max_pairs);
				...
				lcs::Initializer::resize_buffer(device, this->narrow_phase_list_ef, max_intersections);
				lcs::Initializer::resize_buffer(device, this->narrow_phase_list_ef_indices, max_intersections);
			}
...
// !!! Remember to modify the source code by adding inline keyword
