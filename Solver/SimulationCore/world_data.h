...
namespace lcs::Initializer
{
	...
	struct WorldData
	{
		...
		std::vector<float>						fixed_point_stiffness;
		...
		// 0: not oriented, 1: along triangle winding normal, 2: opposite.
		uint face_orientation_along_triangle_winding_mode = 0;

		uint registration_index = std::numeric_limits<uint>::max();
		uint sorted_index = std::numeric_limits<uint>::max();

	private:
		float  applied_auto_scaling_factor = 1.0f;
		float3 auto_scaling_center = luisa::make_float3(0.0f);

		void apply_auto_scaling_if_needed();
		void undo_auto_scaling();
	...
	public:
		...
		WorldData()
			: model_name("unnamed")
			, material_type(Material::MaterialType::Cloth)
		{
		}
		...
		WorldData& set_auto_scaling(const bool enabled);
		WorldData& add_fixed_point_info(const MakeFixedPointsInterface& info, const float stiffness = 1e6f);
		...
		WorldData& set_face_orientation(const bool along_triangle_winding)
		{
			face_orientation_along_triangle_winding_mode = along_triangle_winding ? 1 : 2;
			return *this;
		}
		...
		void set_pinned_verts_from_norm_position(const std::function<bool(const float3&)>& func, const float stiffness, const FixedPointDefaultAnimation& info = FixedPointDefaultAnimation());
		void set_pinned_verts_from_functions(const std::function<bool(uint)>& func, const float stiffness, const FixedPointDefaultAnimation& info = FixedPointDefaultAnimation());
		void set_pinned_verts_from_indices(const std::vector<uint>& indices, const float stiffness, const FixedPointDefaultAnimation& info = FixedPointDefaultAnimation());
