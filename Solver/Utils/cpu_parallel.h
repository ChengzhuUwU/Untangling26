...
namespace CpuParallel
{
	...
	template <typename T, typename ParallelFunc, typename ReduceFuncBinary, bool deterministic_reduce = true>
	...
	{
		T final_value{ zero };
		if constexpr (deterministic_reduce)
		{
			// std::vector<T> thread_result(end_pos - start_pos);
			const uint	   num_blocks = (end_pos - start_pos + blockDim - 1) / blockDim;
			std::vector<T> block_result(num_blocks, zero);
			luisa::fiber::parallel(start_pos,
				end_pos,
				blockDim,
				[&](auto b, auto e)
				{
					...
					uint blockIdx = (b - start_pos) / blockDim;
					block_result[blockIdx] = value;
				});
			return std::reduce(block_result.begin(), block_result.end(), zero, func_binary);
		}
		else
		{
	...
	template <typename T, typename ParallelFunc, bool deterministic_reduce = true>
	inline T parallel_for_and_reduce_sum(uint start_pos, uint end_pos, ParallelFunc func_parallel)
	{
		auto func_binary = [](const T& x, const T& y) -> T
		{ return x + y; };
		return parallel_for_and_reduce<T, ParallelFunc, decltype(func_binary), deterministic_reduce>(
			start_pos,
			end_pos,
			func_parallel,
			func_binary,
