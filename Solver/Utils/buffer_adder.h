...
namespace BufferOp
{
	...
	template <typename T>
	inline void buffer_reset(std::vector<T>& dst)
	{
		std::memset(dst.data(), 0, sizeof(T) * dst.size());
	}

	template <typename T>
	inline void buffer_set(std::vector<T>& dst, const T& value)
	{
		std::fill(dst.begin(), dst.end(), value);
	}

	template <typename T>
	inline void buffer_copy(const std::vector<T>& src, std::vector<T>& dst)
	{
	...
	template <typename T>
	inline void buffer_copy(luisa::compute::Stream& stream,
	...
	template <typename T>
	inline void buffer_upload(luisa::compute::Stream& stream, const std::vector<T>& src, luisa::compute::Buffer<T>& dst)
	{
		if (!dst.valid())
		{
	...
	template <typename T>
	inline void buffer_download(luisa::compute::Stream& stream,
		const luisa::compute::Buffer<T>&				src,
		std::vector<T>&									dst,
	...
	// template <typename T>
	// [[nodiscard]] inline auto buffer_download(const luisa::compute::Buffer<T>& src, std::vector<T>& dst, const bool wait = false)
	// {
	//     if (wait)
	...
	template <typename T>
	inline void buffer_add(const luisa::compute::BufferView<T>& buffer, const Var<uint> dest, const Var<T>& value)
	{
		buffer->write(dest, buffer->read(dest) + value);
	}
	template <typename T>
	inline void buffer_add(const luisa::compute::BufferVar<T>& buffer, const Var<uint> dest, const Var<T>& value)
	{
		buffer->write(dest, buffer->read(dest) + value);
	}

	template <typename T>
	inline void buffer_add(std::vector<T>& buffer, const uint dest, const T& value)
	{
		buffer[dest] = buffer[dest] + value;
	}
	template <typename T>
	inline void buffer_add(const std::span<T>& buffer, const uint dest, const T& value)
	{
		buffer[dest] = buffer[dest] + value;
	}
	template <typename T>
	inline void atomic_buffer_add(const std::span<T>& buffer,
	...
	template <typename T>
	inline void atomic_buffer_add(const std::vector<T>& buffer,
		const std::span<luisa::spin_mutex>&				mutex_ref,
		const uint										dest,
		const T&										value)
	{
	...
	inline void atomic_buffer_add(const luisa::compute::BufferVar<luisa::float3>& buffer,
		const Var<uint>															  dest,
	...
	inline void atomic_buffer_add(const luisa::compute::BufferVar<luisa::float3x3>& buffer,
		const Var<uint>																dest,
