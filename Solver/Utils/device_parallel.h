...
namespace lcs
{
	...
	public:
			...
			size_t							 num_item)
		{
			...
			BufferView<Type4Byte> last_elem_buf = temp_buffer.subview(temp_storage_size - 1, 1).template as<Type4Byte>();
			cmdlist << last_elem_buf.copy_from(d_in.subview(num_item - 1, 1));

			BufferView<Type4Byte> temp_buf = temp_buffer.subview(0, temp_storage_size - 1).template as<Type4Byte>();
