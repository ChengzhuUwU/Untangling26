...
namespace lcs
{
	...
	{
		IntType result = static_cast<IntType>(-1);
		$if(j >= 0 & static_cast<UintType>(j) < num_leaves)
		{
			result = find_common_prefix(mi, get_morton(sa_morton_sorted, j));
		};
		return result;
	}
		...
		const Var<uint>								num_leaves)
	{
		...
		auto	  cp_left = cp_i_j(mi, i - 1, sa_morton_sorted, num_leaves);
		auto	  cp_right = cp_i_j(mi, i + 1, sa_morton_sorted, num_leaves);
	...
	{
		using namespace luisa::compute;
		auto option(compiler.default_option());
...
#define STACKLESS 0
		...
		compiler.compile<1>(fn_query_from_edges_unculled_v2,
			[](BufferVar<CompressedAABB> sa_node_aabb,
				BufferVar<uint2>		 sa_children,
				BufferVar<uint>			 sa_object_idx,
				BufferVar<float3>		 sa_x_begin,
				...
				Uint					 max_count)
			{
				...
				query_template2(sa_node_aabb,
					...
					[&](const Uint adj_fid)
					{
						// device_log("edge query unculled: eid {} adj_fid {}", eid, adj_fid);
						return true;
					});
			});
	...
	void LBVH::broad_phase_query_from_edges_unculled(Stream& stream,
		const Buffer<float3>&								 sa_x_begin,
		const Buffer<float3>&								 sa_x_end,
		const Buffer<uint2>&								 sa_edges,
		const BufferView<uint>&								 broadphase_count,
		...
		const Buffer<float>&								 thickness)
	{
		stream << fn_query_from_edges_unculled_v2(lbvh_data->sa_node_aabb_v2,
			lbvh_data->sa_children,
			lbvh_data->sa_object_idx,
			sa_x_begin,
			sa_x_end,
