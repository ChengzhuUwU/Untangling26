...
namespace lcs // Data IO
{
	...
	constexpr bool ignore_init_penetration = false;
	constexpr bool debug_output_contact = true;
	constexpr uint flag_vert_is_region_match = 1 << 31;
	constexpr uint flag_vert_is_near_contour = 1 << 30;
	constexpr uint flag_vert_is_impact = 1 << 29;
	constexpr uint flag_vert_is_filtered = flag_vert_is_region_match | flag_vert_is_near_contour;
	constexpr uint mask_clear_flags = ~(flag_vert_is_region_match | flag_vert_is_near_contour);
	...
	void NarrowPhasesDetector::resize_buffers(luisa::compute::Device& device, luisa::compute::Stream& stream)
	{
...

		{
			...
			resize_template(device, collision_data->broad_phase_list_vf, static_cast<size_t>(num_vf) * 2u, "broad_phase_list_vf");
			resize_template(device, collision_data->broad_phase_list_ee, static_cast<size_t>(num_ee) * 2u, "broad_phase_list_ee");
		}

		// Resize narrowphase count buffers
		{
			const uint num_pairs = narrow_count[0];
			auto&	   collision_list_buffer = collision_data->narrow_phase_list;
			auto&	   collision_indices_buffer = collision_data->narrow_phase_list_indices;
			if (num_pairs > collision_list_buffer.size())
			{
				const size_t desired_size = static_cast<size_t>(num_pairs) * 2u;
				using TypePair = CollisionPair::CollisionPairTemplate;
				using TypeIndex = uint2;
				constexpr size_t max_buffer_bytes = size_t{ 1u } << 31u;
				if (desired_size > max_buffer_bytes / sizeof(TypePair))
					LUISA_ERROR(
						"Buffer narrow_phase_list requests {} elements ({} bytes), exceeding the 2 GiB per-buffer limit before allocation.",
						desired_size,
						desired_size * sizeof(TypePair));
				if (desired_size > max_buffer_bytes / sizeof(TypeIndex))
					LUISA_ERROR(
						"Buffer narrow_phase_list_indices requests {} elements ({} bytes), exceeding the 2 GiB per-buffer limit before allocation.",
						desired_size,
						desired_size * sizeof(TypeIndex));

				LUISA_INFO("Resize buffer {} : from {} (< 2*CurrMax = {}) to {} ({} MB) , Total collision buffer size = {} MB",
					"narrow_phase_list",
					collision_list_buffer.size(),
					num_pairs * 2,
					desired_size,
					uint(desired_size * sizeof(TypePair) / 1024 / 1024),
					collision_data->get_momery_bytes() / (1024 * 1024));

				LUISA_INFO("Resize buffer {} : from {} (< 2*CurrMax = {}) to {} ({} MB) , Total collision buffer size = {} MB",
					"narrow_phase_list_indices",
					collision_indices_buffer.size(),
					num_pairs * 2,
					desired_size,
					uint(desired_size * sizeof(TypeIndex) / 1024 / 1024),
					collision_data->get_momery_bytes() / (1024 * 1024));
				...
				if (need_store_friction_pairs)
				{
					...
					std::vector<TypePair>  collision_pair_old_data(num_frictions);
					std::vector<TypeIndex> collision_indices_old_data(num_frictions);
					stream << collision_list_buffer.view(0, num_frictions).copy_to(collision_pair_old_data.data())
						   << collision_indices_buffer.view(0, num_frictions).copy_to(collision_indices_old_data.data())
						   << luisa::compute::synchronize();
					collision_list_buffer.release();
					collision_indices_buffer.release();
					collision_list_buffer = device.create_buffer<TypePair>(desired_size);
					collision_indices_buffer = device.create_buffer<TypeIndex>(desired_size);
					stream << collision_list_buffer.view(0, num_frictions).copy_from(collision_pair_old_data.data());
					stream << collision_indices_buffer.view(0, num_frictions).copy_from(collision_indices_old_data.data());
				}
				else
				{
					collision_list_buffer.release();
		...
		// Resize EF intersection buffers
		{
			const uint num_ef_broadphase = broad_count[collision_data->get_ef_count_offset()];
			resize_template(device, collision_data->broad_phase_list_ef, static_cast<size_t>(num_ef_broadphase) * 2u, "broad_phase_list_ef");

			const uint num_ef_narrowphase = narrow_count[1];
			resize_template(device, collision_data->narrow_phase_list_ef, num_ef_narrowphase, "narrow_phase_list_ef");
			resize_template(device, collision_data->narrow_phase_list_ef_indices, num_ef_narrowphase, "narrow_phase_list_ef_indices");
		}
	}
	...
	bool NarrowPhasesDetector::download_broadphase_collision_count(Stream& stream)
	{
		...
		const uint num_ef_broadphase = host_count[collision_data->get_ef_count_offset()];
		return num_vf_broadphase <= collision_data->broad_phase_list_vf.size() / 2
			...
			&& num_ef_broadphase <= collision_data->broad_phase_list_ef.size() / 2;
	...
	void NarrowPhasesDetector::snapshot_friction_pair_count(Stream& stream)
	{
		if (get_scene_params().current_nonlinear_iter == 0)
		{
			...
			stream << device_count.view(0, 1).copy_to(collision_data->num_pairs_in_first_iter)
				   << collision_data->num_pairs_in_first_iter.copy_to(host_collision_data->num_pairs_in_first_iter.data());
		}
	}
	bool NarrowPhasesDetector::download_narrowphase_collision_count(Stream& stream)
	{
		...
		return num_vf_ee <= collision_data->narrow_phase_list.size()
			&& num_ef <= collision_data->narrow_phase_list_ef.size()
...
namespace lcs // CCD
{
	...
	void NarrowPhasesDetector::compile_ccd(AsyncCompiler& compiler)
	{
		...
		compiler.compile<1>(
				...
				Uint					  dispatch_prefix)
			{
				...
				$if((vid & flag_vert_is_filtered) != 0 | (fid & flag_vert_is_filtered) != 0)
				{
				}
				$else
				{
					...
					const auto left_dof_map = sa_x_to_dof.read(vid);
					const auto right_dof_map = sa_x_to_dof.read(face[0]);
					...
					$if(toi != host_accd::line_search_max_t)
					{
						...
						$else
						{
							$if(left_dof_map->is_rigid_body())
							{
								Uint dof_idx = left_dof_map->get_dof_idx();
								sa_toi->atomic(dof_idx).fetch_min(toi);
							}
							$else
							{
								sa_toi->atomic(vid).fetch_min(toi);
							};
							$if(right_dof_map->is_rigid_body())
							{
								Uint dof_idx = right_dof_map->get_dof_idx();
								sa_toi->atomic(dof_idx).fetch_min(toi);
							}
							$else
							{
								sa_toi->atomic(face[0]).fetch_min(toi);
								sa_toi->atomic(face[1]).fetch_min(toi);
								sa_toi->atomic(face[2]).fetch_min(toi);
							};
						};
					};
				};

				if constexpr (debug_output_contact)
				{
					$if(toi != host_accd::line_search_max_t)
					{
						const Uint left_output = vid | flag_vert_is_impact;
						const Uint right_output = fid | flag_vert_is_impact;
						broadphase_list->write(2 * pair_idx + 0, left_output);
						broadphase_list->write(2 * pair_idx + 1, right_output);
						// device_log("VF CCD impact : pair_idx = {}, vid = {}, fid = {}, toi = {}", pair_idx, vid, fid, toi);
					};
				}
		...
		compiler.compile<1>(
				...
				Uint					  dispatch_prefix)
			{
				...
				$if((left & flag_vert_is_filtered) != 0 | (right & flag_vert_is_filtered) != 0)
				{
				}
				$else
				{
					...
					const auto left_dof_map = sa_x_to_dof.read(left_edge[0]);
					const auto right_dof_map = sa_x_to_dof.read(right_edge[0]);
					...
					$else
					{
						...
						$if(toi != host_accd::line_search_max_t)
						{
							...
							$else
							{
								...
								$else
								{
									sa_toi->atomic(left_edge[0]).fetch_min(toi);
									sa_toi->atomic(left_edge[1]).fetch_min(toi);
								};
								$if(right_dof_map->is_rigid_body())
								{
									Uint dof_idx = right_dof_map->get_dof_idx();
								...
								$else
								{
									sa_toi->atomic(right_edge[0]).fetch_min(toi);
									sa_toi->atomic(right_edge[1]).fetch_min(toi);
								};
							};
						};
					};
				...
				if constexpr (debug_output_contact)
				{
					$if(toi != host_accd::line_search_max_t)
					{
						const Uint left_output = left | flag_vert_is_impact;
						const Uint right_output = right | flag_vert_is_impact;
						broadphase_list->write(2 * pair_idx + 0, left_output);
						broadphase_list->write(2 * pair_idx + 1, right_output);
					};
				}
	...
	void NarrowPhasesDetector::vf_ccd_query(Stream& stream,
		...
		const Buffer<VertexToDofMap>&				sa_x_to_dof,
		...
		const Buffer<float>&						thickness)
	{
		...
		auto& host_count = host_collision_data->broad_phase_collision_count;
		bool  use_global_ccd = get_scene_params().use_global_ccd;
...
namespace lcs // DCD
{
	...
	constexpr float min_dist_threshold_sqr = min_distance_threshold * min_distance_threshold;
	...
	void NarrowPhasesDetector::compile_dcd(AsyncCompiler& compiler)
	{
		...
		compiler.compile<1>(
			...
			[offset_vf](BufferVar<uint>							broadphase_count,
				BufferVar<uint>									broadphase_list,
				BufferVar<uint>									narrowphase_count,
				BufferVar<CollisionPair::CollisionPairTemplate> narrowphase_list,
				BufferVar<uint2>								narrow_phase_list_indices,
				BufferVar<float>								toi_per_vert,
				...
				Uint											dispatch_prefix)
			{
				...
				$else
				{
					...
					$if(d2 < square_scalar(thickness + d_hat) & (d2 > min_dist_threshold_sqr | contact_energy_type == uint(ContactEnergyType::Quadratic)))
					{
						...
						// $if(rest_d2 > rest_distance_culling_rate * square_scalar(thickness + d_hat))  // | d2 < 0.5f * rest_d2
						{
							...
							$if(d < min_dist_threshold_sqr)
							{
								normal = make_float3(0.0f, 1.0f, 0.0f); // Avoid NaN normal when very close
							};
							...
							$if(is_nan_scalar(k1) | is_nan_scalar(k2) | is_inf_scalar(k1) | is_inf_scalar(k2))
							{
								device_log("NaN/INF stiffness in DCD VF pair {}-{} : d = {} (d2 = {}), thickness = {}, d_hat = {}, k1 = {}, k2 = {}",
									...
									k1,
									k2);
								toi_per_vert.atomic(0).fetch_min(0.0f);
		...
		compiler.compile<1>(
			...
			[offset_ee](BufferVar<uint>							broadphase_count,
				BufferVar<uint>									broadphase_list,
				BufferVar<uint>									narrowphase_count,
				BufferVar<CollisionPair::CollisionPairTemplate> narrowphase_list,
				BufferVar<uint2>								narrow_phase_list_indices,
		...
		const float											  kappa)
	{
		...
		if (num_vf_broadphase != 0)
		{
			dispatch_large_thread_template(
				[&](uint curr_dispatch_size, uint dispatch_prefix)
				{
					stream << fn_narrow_phase_vf_dcd_query(collision_data->broad_phase_collision_count,
						collision_data->broad_phase_list_vf,
						collision_data->narrow_phase_collision_count,
						collision_data->narrow_phase_list,
						collision_data->narrow_phase_list_indices,
						collision_data->toi_per_vert,
						sa_x,
						sa_rest_x,
						sa_rest_vert_area,
		...
		const float											  kappa)
	{
		...
		if (num_ee_broadphase != 0)
		{
			dispatch_large_thread_template(
				[&](uint curr_dispatch_size, uint dispatch_prefix)
				{
					stream << fn_narrow_phase_ee_dcd_query(collision_data->broad_phase_collision_count,
						collision_data->broad_phase_list_ee,
						collision_data->narrow_phase_collision_count,
						collision_data->narrow_phase_list,
						collision_data->narrow_phase_list_indices,
...
namespace lcs // Compute Contact Gradient & Hessian & Assemble
{
	...
	void NarrowPhasesDetector::device_process_collision_friction(luisa::compute::Stream& stream,
		const Buffer<float3>&																sa_x,
	...
	void NarrowPhasesDetector::download_narrow_phase_list(luisa::compute::Stream& stream)
	{
		const auto& host_count = host_collision_data->narrow_phase_collision_count;
		const uint	num_pairs = host_count.front();
		if (num_pairs == 0)
		...
		if (host_list.size() < num_pairs)
			host_list.resize(num_pairs);
		if (host_list_indices.size() < num_pairs)
			host_list_indices.resize(num_pairs);
		stream << collision_data->narrow_phase_list.view(0, num_pairs)
					  .copy_to(host_list.data())
			   << collision_data->narrow_phase_list_indices.view(0, num_pairs)
					  .copy_to(host_list_indices.data())
			   << luisa::compute::synchronize();
	}

	// consistent_solve host twin of fn_perPair_assemble_gradient_hessian: identical
	// per-pair math, but the pairs are visited in a canonical total order on one
	// thread, so repeated runs produce bitwise-identical cgB/cgA_diag.
	void NarrowPhasesDetector::host_perPair_evaluate_gradient_hessian(
		const std::vector<uint>&	 sa_vert_affine_bodies_id,
		const std::vector<float3>&	 sa_scaled_model_x,
		const uint					 prefix_abd,
		std::vector<float3>&		 sa_cgB,
		std::vector<float3x3>&		 sa_cgA_diag)
	{
		const uint num_pairs = host_collision_data->narrow_phase_collision_count.front();
		if (num_pairs == 0)
			return;
		const auto& pairs = host_collision_data->narrow_phase_list;
		if (pairs.size() < num_pairs)
		{
			LUISA_ERROR("host_perPair_evaluate_gradient_hessian: pair list size {} < count {}",
				pairs.size(), num_pairs);
		}

		static_assert(std::is_trivially_copyable_v<CollisionPair::CollisionPairTemplate>);
		std::vector<uint> order(num_pairs);
		std::iota(order.begin(), order.end(), 0u);
		{
			auto pair_key = [](const CollisionPair::CollisionPairTemplate& pair)
			{
				const uint4 idx = pair.get_indices();
				return std::array<uint, 5>{ pair.get_collision_type(), idx[0], idx[1], idx[2], idx[3] };
			};
			std::sort(order.begin(), order.end(),
				[&](const uint left, const uint right)
				{
					const auto key_left = pair_key(pairs[left]);
					const auto key_right = pair_key(pairs[right]);
					if (key_left != key_right)
						return key_left < key_right;
					// Fully identical pairs are interchangeable, so the payload
					// comparison makes the order total without depending on the
					// GPU atomic slot assignment.
					return std::memcmp(&pairs[left], &pairs[right],
							   sizeof(CollisionPair::CollisionPairTemplate))
						< 0;
				});
		}

		for (uint rank = 0; rank < num_pairs; rank++)
		{
			const uint  pair_idx = order[rank];
			const auto& pair = pairs[pair_idx];
			const uint4 indices = pair.get_indices();
			const float4 weight = pair.get_weight();

			const float  k1 = pair.get_k1();
			const float  k2 = pair.get_k2();
			const float3 normal = pair.get_normal();

			float3	 grad = k1 * normal;
			float3x3 hess = k2 * outer_product(normal, normal);
			{
				const float  lambda = pair.get_friction_mu_lambda();
				const float  friction_eps = Friction::ando_barrier::friction_eps;
				const float3 rel_dx = pair.get_friction_rel_dx();
				const auto   vals = Friction::ipc_barrier::compute_friction_gradient_hessian(
					lambda, normal, rel_dx, friction_eps);
				grad = grad + vals.first;
				hess = hess + vals.second;
			}

			const uint body_left = sa_vert_affine_bodies_id[indices[0]];
			const uint body_right = sa_vert_affine_bodies_id[indices[2]];
			const auto collision_type = pair.get_collision_type();

			// Endpoint expansion identical to CollisionPairTemplate::get_active_indices:
			// a soft side contributes its original template slots (weight = template
			// weight); a rigid side contributes the 4 affine DOF slots with the
			// aligned weight aw[c] = sum_s weight[s] * (1, model_x[s])[c] over that
			// side's slots.
			struct Endpoint
			{
				uint	  dof;
			...
			auto append_soft_side = [&](const uint slot_begin, const uint slot_end)
			{
				for (uint slot = slot_begin; slot <= slot_end; slot++)
				{
					endpoints[endpoint_count].dof = indices[slot];
					endpoints[endpoint_count].w = weight[slot];
					endpoint_count++;
				}
			};
			auto append_rigid_side = [&](const uint slot_begin, const uint slot_end, const uint body_index)
			{
				float4 aw = luisa::make_float4(0.0f);
				for (uint slot = slot_begin; slot <= slot_end; slot++)
					aw += weight[slot] * luisa::make_float4(1.0f, sa_scaled_model_x[indices[slot]]);
				for (uint c = 0; c < 4; c++)
				{
					endpoints[endpoint_count].dof = prefix_abd + 4 * body_index + c;
					endpoints[endpoint_count].w = aw[c];
					endpoint_count++;
				}
			};

			const bool is_ee = collision_type == CollisionPair::type_ee();
			if (body_left == -1u)
				append_soft_side(0, is_ee ? 1 : 0);
			else
				append_rigid_side(0, is_ee ? 1 : 0, body_left);

			if (body_right == -1u)
				append_soft_side(is_ee ? 2 : 1, is_ee ? 3 : 3);
			else
				append_rigid_side(is_ee ? 2 : 1, is_ee ? 3 : 3, body_right);

			for (uint ii = 0; ii < endpoint_count; ii++)
			{
				const uint  dof = endpoints[ii].dof;
				const float w = endpoints[ii].w;
				sa_cgB[dof] -= w * grad;
				sa_cgA_diag[dof] = sa_cgA_diag[dof] + w * w * hess;
			}
		}
	}

	// consistent_solve host twin of the device triplet pipeline (slot assignment +
	// sort + assemble): emit (row, col, w_row * w_col * hess) for every ordered
	// pair of distinct endpoints, then merge duplicates in a canonical
	// (row, col, pair_rank) order on one thread.
	void NarrowPhasesDetector::host_assemble_contact_triplet(luisa::compute::Stream& stream,
		const std::vector<uint>&										   sa_vert_affine_bodies_id,
		const std::vector<float3>&									   sa_scaled_model_x,
		const uint													   prefix_abd)
	{
		auto&	   host_count = host_collision_data->narrow_phase_collision_count;
		const uint num_pairs = host_count.front();
		auto&	   host_triplets = host_collision_data->triplet_data.sa_cgA_contact_offdiag_triplet;

		if (num_pairs == 0)
		{
			host_count[CollisionPair::CollisionCount::total_adj_verts_offset()] = 0;
			return;
		}
		const auto& pairs = host_collision_data->narrow_phase_list;
		...
		entries.reserve(static_cast<size_t>(num_pairs) * 12);
		...
		for (uint rank = 0; rank < num_pairs; rank++)
		{
			...
			for (uint ii = 0; ii < endpoint_count; ii++)
			{
				for (uint jj = 0; jj < endpoint_count; jj++)
				{
					if (ii == jj)
						continue;
					if (endpoints[ii].dof == endpoints[jj].dof)
						continue; // Self-loop triplet; the device pipeline asserts these do not occur.
					entries.push_back({ endpoints[ii].dof,
						endpoints[jj].dof,
						rank,
						endpoints[ii].w * endpoints[jj].w * hess });
				}
			}
		}

		std::sort(entries.begin(), entries.end(),
			[](const TripletEntry& left, const TripletEntry& right)
			{
		...
		{
			size_t j = i;
			float3x3 sum = entries[i].mat;
			for (j = i + 1; j < entries.size()
				&& entries[j].row == entries[i].row
				&& entries[j].col == entries[i].col;
				 j++)
			{
				sum = sum + entries[j].mat;
			}
			uint property = MatrixTriplet::is_valid();
			if (i == 0
				|| entries[i - 1].row != entries[i].row)
				property |= MatrixTriplet::is_first_col_in_row();
			if (j == entries.size()
				|| entries[j].row != entries[i].row)
				property |= MatrixTriplet::is_last_col_in_row();
			if (merged_count >= host_triplets.size())
				host_triplets.resize(merged_count + 1);
			host_triplets[merged_count] = make_matrix_triplet(entries[i].row, entries[i].col, property, sum);
			merged_count++;
			i = j;
		}

		host_count[CollisionPair::CollisionCount::total_adj_verts_offset()] = merged_count;

		// Mirror the host triplets and count to the device so downstream device
		// consumers observe the same assembly.
		if (merged_count != 0)
		{
			const auto& device_triplets = collision_data->triplet_data.sa_cgA_contact_offdiag_triplet;
			if (device_triplets.size() >= merged_count)
			{
				stream << device_triplets.view(0, merged_count)
							  .copy_from(host_triplets.data());
			}
			else
			{
				LUISA_WARNING("consistent_solve: device contact triplet buffer ({}) smaller than host assembly ({}); skipping device mirror",
					device_triplets.size(),
					merged_count);
			}
		}
		stream << collision_data->narrow_phase_collision_count.view(0, 1)
					  .copy_from(host_count.data())
			   << luisa::compute::synchronize();
	}
		...
		Buffer<float3x3>&																		sa_cgA_diag)
	{
		...
		device_process_collision_friction(stream, sa_x, sa_x_step_start, sa_vert_friction_coeff, d_hat, thickness);
