...
namespace lcs
{
	...
	public:
		...
		void  snapshot_friction_pair_count(Stream& stream);
	...
	private:
			...
			const std::string						 name)
		{
			if (curr_count > buffer.size())
			{
				...
				if (curr_count > std::numeric_limits<size_t>::max() / 2u)
					LUISA_ERROR("Buffer {} requested element count {} overflows the doubled allocation size.", name, curr_count);
				const size_t desired_size = curr_count * 2u;
				if (desired_size > max_buffer_bytes / sizeof(T))
					LUISA_ERROR(
						"Buffer {} requests {} elements ({} bytes), exceeding the 2 GiB per-buffer limit before allocation.",
						name,
						desired_size,
						desired_size * sizeof(T));
				...
				LUISA_INFO("Resize buffer {} : from {} (< 2*CurrMax = {}) to {} ({} MB) , Total collision buffer size = {} MB",
					...
					curr_count * 2,
					desired_size,
					size_t(desired_size * sizeof(T) / 1024 / 1024),
	...
	public:
		...
		// consistent_solve support (CPU physics path): friction state is computed
		// per pair on the device (each pair writes only its own slot, so values are
		// order-independent), while the gradient/Hessian accumulation itself runs on
		// the host in a canonical pair order.
		void device_process_collision_friction(Stream& stream,
			const Buffer<float3>&							  sa_x,
			const Buffer<float3>&							  sa_x_step_start,
			const Buffer<float>&							  sa_vert_friction_coeff,
			const Buffer<float>&							  d_hat,
			const Buffer<float>&							  thickness);
		void download_narrow_phase_list(Stream& stream);
		void host_perPair_evaluate_gradient_hessian(const std::vector<uint>&	 sa_vert_affine_bodies_id,
			const std::vector<float3>&						 sa_scaled_model_x,
			const uint										 prefix_abd,
			std::vector<float3>&							 sa_cgB,
			std::vector<float3x3>&							 sa_cgA_diag);
		void host_assemble_contact_triplet(luisa::compute::Stream& stream,
	...
	private:
		...
		luisa::compute::Shader<1,
			...
			luisa::compute::Buffer<VertexToDofMap>,
			luisa::compute::Buffer<float>,
			luisa::compute::Buffer<float>,
			bool,
			uint>
