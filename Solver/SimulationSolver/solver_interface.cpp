...
{
	void check_directory_exists(const std::string_view& full_path);

	namespace
	{
		struct ContourExportSide
		{
			uint			  object_id = std::numeric_limits<uint>::max();
			std::vector<uint> seed_vertices;
		};

		struct ContourExportSelection
		{
			uint			  side_idx = 0u;
			uint			  registration_id = std::numeric_limits<uint>::max();
			std::vector<uint> selected_vertices;
			std::vector<uint> selected_faces;
		};

		void sort_unique_ids(std::vector<uint>& ids)
		{
			std::sort(ids.begin(), ids.end());
			ids.erase(std::unique(ids.begin(), ids.end()), ids.end());
		}

		std::vector<uint> collect_vertices_within_rings(const lcs::MeshData<std::vector>* host_mesh_data,
			const uint																	  object_id,
			const std::vector<uint>&													  seed_vertices,
			const uint																	  ring_count)
		{
			if (object_id >= host_mesh_data->num_meshes)
				return {};
			...
			std::vector<uint> frontier_seeds;
			frontier_seeds.reserve(seed_vertices.size());
			for (const uint vid : seed_vertices)
				if (vid >= prefix_vid && vid < suffix_vid)
					frontier_seeds.push_back(vid);
			sort_unique_ids(frontier_seeds);
			if (frontier_seeds.empty())
				return {};

			std::unordered_map<uint, uint> hop_dist;
			hop_dist.reserve(frontier_seeds.size() * 4u + 1u);
			std::queue<uint> frontier;
			for (const uint vid : frontier_seeds)
			{
				hop_dist.emplace(vid, 0u);
				frontier.push(vid);
			}

			while (!frontier.empty())
			{
				const uint vid = frontier.front();
				frontier.pop();
				const uint curr_hop = hop_dist.find(vid)->second;
				if (curr_hop >= ring_count)
					continue;

				for (const uint eid : host_mesh_data->vert_adj_edges[vid])
				{
					const luisa::uint2 edge = host_mesh_data->sa_edges[eid];
					const uint		   neighbors[2] = { edge.x, edge.y };
					for (const uint neighbor_vid : neighbors)
					{
						if (neighbor_vid == vid || neighbor_vid < prefix_vid || neighbor_vid >= suffix_vid)
							continue;
						if (hop_dist.find(neighbor_vid) != hop_dist.end())
							continue;
						hop_dist.emplace(neighbor_vid, curr_hop + 1u);
						frontier.push(neighbor_vid);
					}
				}
			}

			std::vector<uint> selected_vertices;
			selected_vertices.reserve(hop_dist.size());
			for (const auto& [vid, _] : hop_dist)
				selected_vertices.push_back(vid);
			sort_unique_ids(selected_vertices);
			return selected_vertices;
		}

		std::vector<uint> collect_faces_for_vertices(const lcs::MeshData<std::vector>* host_mesh_data,
			const uint																   object_id,
			const std::vector<uint>&												   selected_vertices)
		{
			if (object_id >= host_mesh_data->num_meshes || selected_vertices.empty())
				return {};
			...
			std::unordered_set<uint> selected_vertex_set(selected_vertices.begin(), selected_vertices.end());
			std::unordered_set<uint> seen_faces;
			seen_faces.reserve(selected_vertices.size() * 2u + 1u);
			std::vector<uint> selected_faces;
			selected_faces.reserve(selected_vertices.size());

			for (const uint vid : selected_vertices)
			{
				for (const uint fid : host_mesh_data->vert_adj_faces[vid])
				{
					if (fid < prefix_fid || fid >= suffix_fid)
						continue;
					if (!seen_faces.insert(fid).second)
						continue;

					const luisa::uint3 face = host_mesh_data->sa_faces[fid];
					if (selected_vertex_set.find(face.x) != selected_vertex_set.end()
						&& selected_vertex_set.find(face.y) != selected_vertex_set.end()
						&& selected_vertex_set.find(face.z) != selected_vertex_set.end())
					{
						selected_faces.push_back(fid);
					}
				}
			}

			sort_unique_ids(selected_faces);
			return selected_faces;
		}

		void write_combined_submeshes_to_obj(const std::string_view& full_path,
			const std::vector<luisa::float3>&						 positions,
			const std::vector<luisa::uint3>&						 faces,
			const std::vector<ContourExportSelection>&				 export_selections,
			...
			const uint												 ring_count)
		{
			...
			if (!file.is_open())
				throw std::runtime_error(luisa::format("Unable to open file: {}", full_path));

			file << "# contour submesh export" << std::endl;
			file << "o contour_" << contour_idx << "_ring_" << ring_count << std::endl;

			uint global_vertex_offset = 0u;
			for (const auto& selection : export_selections)
			{
				file << "# side " << selection.side_idx << ", mesh " << selection.registration_id << std::endl;

				std::unordered_map<uint, uint> global_to_local_vid;
				global_to_local_vid.reserve(selection.selected_vertices.size() * 2u + 1u);
				for (uint local_vid = 0u; local_vid < selection.selected_vertices.size(); ++local_vid)
				{
					const uint vid = selection.selected_vertices[local_vid];
					global_to_local_vid.emplace(vid, global_vertex_offset + local_vid + 1u);
					const auto vertex = positions[vid];
					file << "v " << vertex.x << " " << vertex.y << " " << vertex.z << std::endl;
				}

				for (const uint fid : selection.selected_faces)
				{
					const luisa::uint3 face = faces[fid];
					const auto		   it0 = global_to_local_vid.find(face.x);
					const auto		   it1 = global_to_local_vid.find(face.y);
					const auto		   it2 = global_to_local_vid.find(face.z);
					if (it0 == global_to_local_vid.end() || it1 == global_to_local_vid.end() || it2 == global_to_local_vid.end())
						continue;
					file << "f " << it0->second << " " << it1->second << " " << it2->second << std::endl;
				}

				global_vertex_offset += static_cast<uint>(selection.selected_vertices.size());
			}
		}
	} // namespace
	...
	void SolverInterface::set_data_pointer(SolverData& solver_data, SolverHelper& solver_helper)
	{
		...
		this->host_untangling_data = &solver_data.host_untangling_data;
		this->untangling_data = &solver_data.untangling_data;
		...
		this->intersection_resolver = &solver_helper.intersection_resolver;
		...
		luisa::compute::Stream&								stream)
	{
...

		{
			...
			host_untangling_data->resize_collision_data_list(device,
				host_mesh_data->num_verts,
				host_mesh_data->num_faces,
				host_mesh_data->num_edges,
				host_sim_data->num_dof,
			...
			untangling_data->resize_collision_data_list(device,
				host_mesh_data->num_verts,
				host_mesh_data->num_faces,
				host_mesh_data->num_edges,
				host_sim_data->num_dof,
	...
	{
		LUISA_INFO("JIT Compiling Energy Kernels...");
		{
			compile_compute_energy(compiler);
		}

		LUISA_INFO("JIT Compiling LBVH construction kernels...");
		{
		...
		LUISA_INFO("JIT Compiling Intersection Resolver...");
		{
			intersection_resolver->set_collision_data(
				collision_data, host_collision_data, untangling_data, host_untangling_data, mesh_data, host_mesh_data, sim_data, host_sim_data);
			intersection_resolver->set_lbvh_data(lbvh_data_face, lbvh_data_edge);
			intersection_resolver->compile(compiler);
		}
	}
	...
	void SolverInterface::get_visualize_curves(std::vector<std::array<float, 3>>& output_positions,
		std::vector<std::array<uint, 2>>&										  output_edges,
		std::vector<std::array<float, 3>>&										  output_colors)
	{
		intersection_resolver->get_intersection_curves(output_positions, output_edges, output_colors);
	}
	...
	void SolverInterface::get_edges_to_host(std::vector<std::vector<std::array<uint, 2>>>& output_edges)
	{
		const auto& edges = host_mesh_data->sa_edges;

		if (output_edges.size() != host_mesh_data->num_meshes)
		{
			output_edges.resize(host_mesh_data->num_meshes);
		}

		for (uint sortedIdx = 0; sortedIdx < host_mesh_data->num_meshes; sortedIdx++)
		{
			const uint meshIdx = host_mesh_data->sorted_to_input_mesh_id[sortedIdx];
			const uint prefix_edge = host_mesh_data->prefix_num_edges[sortedIdx];
			const uint suffix_edge = host_mesh_data->prefix_num_edges[sortedIdx + 1];
			const uint num_edges = suffix_edge - prefix_edge;
			const uint prefix_vert = host_mesh_data->prefix_num_verts[sortedIdx];

			if (output_edges[meshIdx].size() != num_edges)
			{
				output_edges[meshIdx].resize(num_edges);
			}

			for (uint eid = 0; eid < num_edges; eid++)
			{
				auto edge = edges[prefix_edge + eid];
				output_edges[meshIdx][eid] = {
					edge.x - prefix_vert,
					edge.y - prefix_vert,
				};
			}
	...
	void SolverInterface::save_current_frame_state_file(const std::string_view& full_path)
	{
	...
	uint SolverInterface::query_sorted_index_from_registration_id(uint registration_id) const
	{
		if (registration_id >= host_mesh_data->input_to_sorted_mesh_id.size())
		{
			LUISA_WARNING("Invalid registration_id {}. Out of range.", registration_id);
			return -1u;
		}
		return host_mesh_data->input_to_sorted_mesh_id[registration_id];
	}

	uint SolverInterface::query_local_vid_from_global_vid(const uint global_vid) const
	{
		const auto& mapping = host_mesh_data->sa_global_vid_to_local_vid;
		return (global_vid < mapping.size()) ? mapping[global_vid] : -1u;
	}

	uint SolverInterface::query_registration_id_from_global_vid(const uint global_vid) const
	{
		const auto& mapping = host_mesh_data->sa_vert_mesh_id;
		const uint	sorted_idx = (global_vid < mapping.size()) ? mapping[global_vid] : -1u;
		return query_registration_id_from_sorted_index(sorted_idx);
	}

	uint SolverInterface::query_registration_id_from_sorted_index(uint sorted_index) const
	{
		if (sorted_index >= host_mesh_data->num_meshes)
		{
			LUISA_WARNING("Invalid sorted index {}. Out of range.", sorted_index);
			return -1u;
		}
		return host_mesh_data->sorted_to_input_mesh_id[sorted_index];
	}

	uint SolverInterface::query_global_vid_from_registration_id_and_local_vid(uint registration_id, uint local_vid) const
	{
		...
		const uint num_verts = suffix - prefix;
		if (local_vid >= num_verts)
		{
			LUISA_WARNING("Invalid local_vid {} for registration_id {}. Out of range.", local_vid, registration_id);
			return -1u;
		}
		return prefix + local_vid;
	}

	std::vector<uint> SolverInterface::query_vert_adj_edges(const uint global_vid) const
	{
		if (global_vid >= host_mesh_data->vert_adj_edges.size())
		{
			LUISA_WARNING("Invalid global_vid {}. Out of range.", global_vid);
			return {};
		}
		return host_mesh_data->vert_adj_edges[global_vid];
	}
	...
	void SolverInterface::set_object_sim_result_by_registration_id(uint registration_id,
		const std::vector<std::array<float, 3>>&						input_positions)
	{
		...
		if (input_positions.size() != num_verts)
		{
			LUISA_ERROR("Input vertex count {} does not match the number of vertices {} for the mesh with registration id {}.",
				input_positions.size(), num_verts, registration_id);
			return;
		}

		for (uint vid = 0; vid < num_verts; vid++)
		{
			const auto	 pos = input_positions[vid];
			const float3 new_pos = { pos[0], pos[1], pos[2] };
			const uint	 global_vid = prefix_vert + vid;
			host_sim_data->sa_x_outer[global_vid] = new_pos;

			// Also update sa_q_outer so the solver picks up the new positions
			const auto& map_info = host_sim_data->sa_x_to_dof_map[global_vid];
			const uint	dof_idx = map_info.get_dof_idx();
			if (map_info.is_soft_body())
			{
				host_sim_data->sa_q_outer[dof_idx] = new_pos;
			}
		}
	}

	void SolverInterface::load_state_from_state_file(const std::string_view& full_path)
	{
	...
	void SolverInterface::load_state_from_obj_file(const std::string_view& full_path)
	{
		std::ifstream file(std::string(full_path), std::ios::in);
		if (!file.is_open())
		{
			LUISA_ERROR("Unable to open OBJ file: {}", full_path);
			return;
		}
		if (host_sim_data->sa_x_outer.size() != host_sim_data->sa_q_outer.size())
		{
			LUISA_WARNING("Size of position buffer and q buffer do not match in sim data. Cannot load OBJ file.");
			return;
		}

		std::vector<float3> loaded_positions;
		...
		while (std::getline(file, line))
		{
			...
			if (line[0] == 'v')
			{
				...
				float			   x, y, z;
				iss >> x >> y >> z;
				loaded_positions.push_back({ x, y, z });
			}
		}
		file.close();

		if (loaded_positions.size() != host_sim_data->num_verts_total)
		{
			LUISA_ERROR("Loaded vertex count {} does not match the number of vertices {} in the sim data.",
				loaded_positions.size(), host_sim_data->num_verts_total);
			return;
		}

		buffer_copy(loaded_positions, host_sim_data->sa_x_outer);
		buffer_copy(loaded_positions, host_sim_data->sa_q_outer);

		LUISA_INFO("OBJ file loaded: {}", full_path);
	}
	...
	std::vector<std::string> SolverInterface::export_contour_submeshes(const std::string_view& output_dir,
		uint																				   contour_idx,
		uint																				   ring_count)
	{
		const auto& contours = host_untangling_data->intersection_contours;
		if (contours.empty())
			throw std::runtime_error("No intersection contours available. Run collision detection or a simulation step first.");
		if (contour_idx >= contours.size())
			throw std::runtime_error(luisa::format("Contour index {} is out of range [0, {}).", contour_idx, contours.size()));

		const auto& contour = contours[contour_idx];
		if (contour.empty())
			throw std::runtime_error(luisa::format("Contour {} is empty.", contour_idx));
		...
		if (ef_pair_mesh_index.size() < ef_list.size())
			throw std::runtime_error("Contour export requires ef_pair_mesh_index to be populated on host.");

		std::array<ContourExportSide, 2> export_sides;
		for (const uint pair_idx : contour)
		{
			if (pair_idx >= ef_list.size() || pair_idx >= ef_pair_mesh_index.size())
				throw std::runtime_error(luisa::format("Contour {} references invalid pair index {}.", contour_idx, pair_idx));

			const auto& pair = ef_list[pair_idx];
			const uint	edge_side = ef_pair_mesh_index[pair_idx];
			if (edge_side > 1u)
				throw std::runtime_error(luisa::format("Contour {} pair {} has invalid side index {}.", contour_idx, pair_idx, edge_side));
			const uint face_side = edge_side ^ 1u;

			const luisa::uint2 edge = pair.get_edge();
			const luisa::uint3 face = pair.get_face();
			const uint		   edge_object_id = host_mesh_data->sa_vert_mesh_id[edge.x];
			const uint		   face_object_id = host_mesh_data->sa_vert_mesh_id[face.x];

			auto assign_side_object = [&](ContourExportSide& side, const uint object_id, const uint side_idx, const char* label)
			{
				if (side.object_id == std::numeric_limits<uint>::max())
				{
					side.object_id = object_id;
					return;
				}
				if (side.object_id != object_id)
					throw std::runtime_error(luisa::format(
						"Contour {} mixes multiple {} objects on side {}: {} vs {}.",
						contour_idx,
						label,
						side_idx,
						side.object_id,
			...
			assign_side_object(export_sides[edge_side], edge_object_id, edge_side, "edge");
			assign_side_object(export_sides[face_side], face_object_id, face_side, "face");

			export_sides[edge_side].seed_vertices.insert(export_sides[edge_side].seed_vertices.end(), { edge.x, edge.y });
			export_sides[face_side].seed_vertices.insert(export_sides[face_side].seed_vertices.end(), { face.x, face.y, face.z });
		}

		std::vector<ContourExportSelection> export_selections;
		export_selections.reserve(2u);
		const std::string output_dir_str = output_dir.empty() ? std::string(".") : std::string(output_dir);
		for (uint side_idx = 0u; side_idx < 2u; ++side_idx)
		{
			auto& side = export_sides[side_idx];
			if (side.object_id == std::numeric_limits<uint>::max())
				throw std::runtime_error(luisa::format("Contour {} side {} does not resolve to a mesh.", contour_idx, side_idx));

			sort_unique_ids(side.seed_vertices);
			ContourExportSelection selection;
			selection.side_idx = side_idx;
			selection.registration_id = query_registration_id_from_sorted_index(side.object_id);
			selection.selected_vertices = collect_vertices_within_rings(host_mesh_data, side.object_id, side.seed_vertices, ring_count);
			selection.selected_faces = collect_faces_for_vertices(host_mesh_data, side.object_id, selection.selected_vertices);
			export_selections.push_back(std::move(selection));
		}

		const std::string full_path = luisa::format(
			"{}/export_contour_{}_ring_{}.obj",
			output_dir_str,
			contour_idx,
			ring_count);
		write_combined_submeshes_to_obj(full_path,
			host_sim_data->sa_x_outer,
		...
		saved_paths.push_back(full_path);
		return saved_paths;
	}
	...
	void SolverInterface::compile_compute_energy(AsyncCompiler& compiler)
	{
		...
		inertia_energy =
			std::make_unique<SoftInertiaEnergy>(
				sim_data->sa_q_tilde.view(), sim_data->sa_q_is_fixed.view(), sim_data->sa_system_energy.view());
		...
		abd_inertia_energy =
			std::make_unique<AbdInertiaEnergy>(
				sim_data->sa_q_tilde.view(), sim_data->sa_q_is_fixed.view(), sim_data->sa_system_energy.view());
	...
	{
		solver_helper.intersection_resolver.reset_prp_shaders();
