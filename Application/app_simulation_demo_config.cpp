...
namespace Demo::Simulation
{
	...
	void load_scene_params_from_json(const std::function<void(const lcs::Initializer::WorldData&)>& fn_register_mesh, const std::string& json_path)
	{
		...
		get_bool("use_global_ccd", lcs::get_scene_params().use_global_ccd);
		get_bool("use_energy_linesearch", lcs::get_scene_params().use_energy_linesearch);
		get_bool("print_pcg_info", lcs::get_scene_params().print_pcg_info);
		get_bool("print_collision_info", lcs::get_scene_params().print_collision_info);
		lcs::get_scene_params().print_pcg_info = true;
		lcs::get_scene_params().print_collision_info = true;
		...
		get_int("load_state_frame", reinterpret_cast<int&>(lcs::get_scene_params().load_state_frame));
			...
			yyjson_arr_foreach(shells, i, n, shell_val)
			{
				...
				if (fp_arr && yyjson_is_arr(fp_arr))
				{
					...
					yyjson_arr_foreach(fp_arr, j, jm, fpv)
					{
						...
						info.add_fixed_point_info(mfp, 1e6f);
					}
				}
